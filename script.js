...
(function () {
  ...
  function buildTrioCard(p) {
    const card = el('div', 'trio-card' + (p.needsReview ? ' needs-review' : ''));
    card.tabIndex = 0;
    card.setAttribute('role', 'button');
    ...
    img.loading = 'lazy';
    media.appendChild(img);
    if (p.badge) media.appendChild(el('span', 'card__badge', escapeHtml(p.badge)));
    if (p.people) media.appendChild(el('span', 'card__people', escapeHtml(p.people)));
    card.appendChild(media);
    ...
    } else if (p.ingredients && p.ingredients.length) {
      body.appendChild(el('p', 'trio-card__desc', escapeHtml(p.ingredients.join(' · '))));
    }
    if (p.options && p.options.length) {
      ...
      const footer = el('div', 'trio-card__prices trio-card__prices--single');
      footer.appendChild(el('span', 'trio-card__price-val', escapeHtml(priceLabelFor(p))));
      body.appendChild(footer);
    }
    if (p.needsReview) body.appendChild(el('span', 'pill-note', 'Prix à confirmer'));
  ...
  function buildCategorySection(cat) {
    ...
    /* Style unifié sur tout le menu (demande d'Aya, 15/09/2026) : chaque
       catégorie affiche désormais ses produits avec le même traitement
       "grande photo + nom" que Twister & Burgers, au lieu du mélange
       précédent (stage flottant pour certaines catégories, grille simple
       pour Box/Big Box). */
    const grid = el('div', 'trio-grid reveal');
    products.forEach((p) => grid.appendChild(buildTrioCard(p)));
  ...
  function openModal(product) {
    ...
    if (product.variants && product.variants.length) {
      /* Grouped product (ex: "Soda") — une fiche dans le menu, et ici une
         liste des boissons ; cliquer sur l'une d'elles l'affiche seule
         (photo + nom + prix) au-dessus, comme demandé par Aya. */
      ...
      const showVariant = (variant) => {
        if (variant) {
          ...
          modalPrice.textContent = variant.price != null ? variant.price + ' DH' : 'Prix à confirmer';
        } else {
      ...
      product.variants.forEach((variant) => {
        const btn = el('button', 'modal__variant');
        btn.type = 'button';
        const media = el('div', 'modal__variant__media');
        const img = el('img');
        ...
        btn.appendChild(el('span', 'modal__variant__name', escapeHtml(variant.name)));
        btn.addEventListener('click', () => {
          modalVariants.querySelectorAll('.modal__variant').forEach((b) => b.classList.remove('is-selected'));
